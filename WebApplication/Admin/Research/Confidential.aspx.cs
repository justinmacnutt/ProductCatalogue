...
namespace WebApplication.Admin.Research
{
    public partial class Confidential : System.Web.UI.Page
    {
        ...
        public static List<ProductVo> GetProducts(string productName)
        {
            ...
            return researchBs.GetAccommodations(productName).Select(p => new ProductVo {licenseNumber = p.licenseNumber, productId = p.id, productName = p.productName}).ToList();
        }
        ...
        public static void ValidateForm(string productName, string licenseNumber)
        {
            ...
            {
                var pl = researchBs.GetProductsByLicenseNumber(new List<string> { licenseNumber }, ProductType.Accommodation);
                if (pl.Count() != 1)
                {
        ...
        [WebMethod]
        public static string HelloJustin(string productName)
        {
            return String.Format("Hello {0}", productName);
        }
    }
}